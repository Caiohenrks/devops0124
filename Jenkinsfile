    ...
    stages {
        ...
        stage('Build Images') {
            steps {
                script {
                    // Nome das imagens
                    def imageNameWeb = 'caiohenrks/web'
                    def imageNameDB = 'caiohenrks/db'
                    def imageNameNginx = 'caiohenrks/nginx'

                    // Executa o build das imagens com os respectivos Dockerfiles
                    docker.withRegistry('https://registry.hub.docker.com', 'dockerhubcaiohenrks') {
